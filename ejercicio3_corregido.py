...
def encontrar_puntos_contacto(contorno, tol_altura=5):
    ...
    if len(puntos_base_idx) < 2:
        # Fallback: usar extremos en X
        idx_izq = np.argmin(contorno[:, 1])
        idx_der = np.argmax(contorno[:, 1])
    else:
    ...
    # Asegurar que idx_izq < idx_der para mantener orden del contorno
    if idx_izq > idx_der:
...
def calcular_perimetro(contorno, escala):
    ...
    try:
        # Verificar si el contorno es válido (sin saltos grandes)
        diffs = np.sqrt(np.diff(contorno[:, 1]) ** 2 + np.diff(contorno[:, 0]) ** 2)
        max_diff = np.max(diffs)
        mean_diff = np.mean(diffs)
        
        # Si hay saltos muy grandes comparados con la media, usar método simple
        if max_diff > 5 * mean_diff and max_diff > 50:
            print(f"Advertencia: Detectado salto grande en contorno ({max_diff:.1f}), usando cálculo simple")
            return np.sum(diffs) * escala
        
        # Usar interpolación cúbica solo si el contorno es suave
        t = np.linspace(0, 1, len(contorno))
        cs_x = CubicSpline(t, contorno[:, 1], bc_type='natural')
        cs_y = CubicSpline(t, contorno[:, 0], bc_type='natural')

        t_fine = np.linspace(0, 1, 3 * len(contorno))  # Reducir densidad para evitar problemas
        dx_dt = cs_x(t_fine, 1)
        dy_dt = cs_y(t_fine, 1)

        perimetro = np.trapz(np.sqrt(dx_dt ** 2 + dy_dt ** 2), t_fine) * escala
        
        # Verificación de sanidad: el perímetro interpolado no debería ser muy diferente del simple
        perimetro_simple = np.sum(diffs) * escala
        if abs(perimetro - perimetro_simple) / perimetro_simple > 0.5:  # Más de 50% de diferencia
            print(f"Advertencia: Gran diferencia entre métodos, usando cálculo simple")
            return perimetro_simple
...
def dividir_contorno_mejorado(contorno, idx_izq, idx_der):
    """
    Divide el contorno en mitades izquierda y derecha de manera más robusta.
    
    Args:
        contorno: Array [N, 2] con coordenadas [y, x]
        idx_izq: Índice del punto de contacto izquierdo
        idx_der: Índice del punto de contacto derecho
    
    Returns:
        tuple: (contorno_izq, contorno_der)
    """
    n_puntos = len(contorno)
    
    # Asegurar que idx_izq < idx_der
    if idx_izq > idx_der:
        idx_izq, idx_der = idx_der, idx_izq
    
    # Método 1: División directa si los puntos están bien separados
    if idx_der - idx_izq > n_puntos // 4:  # Al menos 25% del contorno entre puntos
        contorno_izq = contorno[idx_izq:idx_der + 1]
        
        # Para el contorno derecho, crear un path continuo
        if idx_der < n_puntos - 1:
            # Tomar desde idx_der hasta el final, luego desde el inicio hasta idx_izq
            parte1 = contorno[idx_der:]
            parte2 = contorno[:idx_izq + 1]
            contorno_der = np.vstack([parte1, parte2])
        else:
            # El punto derecho está al final, solo tomar hasta idx_izq
            contorno_der = contorno[:idx_izq + 1]
            
    else:
        # Método 2: División por posición X relativa al centroide
        centro_x = np.mean(contorno[:, 1])
        
        # Encontrar el punto más alto (ápice de la gota)
        idx_apice = np.argmin(contorno[:, 0])
        
        # Dividir el contorno en dos partes desde el ápice
        # Parte 1: desde ápice en una dirección
        # Parte 2: desde ápice en la otra dirección
        
        if idx_apice < n_puntos // 2:
            # Ápice en primera mitad
            parte1 = contorno[idx_apice:idx_apice + n_puntos // 2]
            parte2 = np.vstack([contorno[idx_apice + n_puntos // 2:], 
                               contorno[:idx_apice + 1]])
        else:
            # Ápice en segunda mitad
            parte1 = np.vstack([contorno[idx_apice:], 
                               contorno[:idx_apice - n_puntos // 2 + 1]])
            parte2 = contorno[idx_apice - n_puntos // 2:idx_apice + 1]
        
        # Asignar según posición X promedio
        if np.mean(parte1[:, 1]) < centro_x:
            contorno_izq, contorno_der = parte1, parte2
        else:
            contorno_izq, contorno_der = parte2, parte1
...
def calcular_propiedades_geometricas(df, escala=4.13e-6, densidad=7380):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            # Usar la nueva función de división mejorada
            ...
            # Calcular simetría con verificación de división por cero
